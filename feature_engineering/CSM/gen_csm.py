...
handler2 = logging.FileHandler('./logs/gen_csm.log', mode='w')
...
with open('./CSM/AA_atom_pharm_dict.json', 'r') as file:
...
def extractResidueEnvironment(pdb_id, mutation, chain, path2pdbs, res_env_radius, pdb_graph_dict) -> pd.DataFrame:
    ...
    env_path = f'./CSM/res_env/{pdb_id}_{mutation}_env.pkl'
    ...
    # Обработка строки mutation (F149A)
    aa, num_aa, mut_aa = re.match(r"([A-Za-z])(\d+[A-Za-z]*)([A-Za-z])", mutation).groups() #F149A –> F+149+A
    aa = protein_letters_1to3[aa].upper() # F –> Phe –> PHE
    mut_aa = protein_letters_1to3[mut_aa].upper() # F –> Phe –> PHE
    ...
    if pdb_id not in pdb_graph_dict.keys():
        try:
            graph = construct_graph(config=config, path=f'{path2pdbs}/{pdb_id}.pdb')
            pdb_graph_dict[f'{pdb_id}from{dataset}'] = graph
        ...
        graph = pdb_graph_dict[f'{pdb_id}from{dataset}']
